...
const handleError = (res: express.Response, error: Error | string, message = "Internal server error") =>
{
  console.error(error);
...
async function main()
{
  ...
  const app = express();// added for upload
  ...
  app.use(express.json());// added for upload
  ...
  // Cleaning function
  const cleanupFiles = async (originalFilePath: string, processedFilePath: string, thumbnailFilePath?: string) =>
  {
  ...
  router.get("/tmp_canva", authenticateUser, async (req, res) =>
  {
    ...
    {
      handleError(res, error as Error, "Error handling frictionless authentication");
    }
  });
  ...
  router.post("/download", authenticateUser, download.single("media"), async (req, res) =>
  {
    ...
    try
    {
      ...
      const inputFilePath = path.join(TMP_CANVA_DIR, req.file.filename);
      const outputFilePath = path.join(PROCESSED_DIR, processFile(req.file.filename)); // '../../../../../tmp_canva' for local
      ...
      try
      {
        ...
        */
        ...
        if (isVideo)
        {
          ...
          const outputFirstFrameFilePath = path.join(PROCESSED_DIR, `thumbnail_${fileWithoutExtension}.jpg`);
          ...
          try
          {
            ...
            const thumbnailFileUrl = `${req.protocol}://canva.spooqs.com/tmp/${path.basename(outputFirstFrameFilePath)}`;
            const fileUrl = `${req.protocol}://canva.spooqs.com/tmp/${path.basename(outputFilePath)}`;
          ...
          } catch (frameError)
          {
            ...
            return handleError(res, frameError as Error, "Error extracting frame from video.");
          }
        } else
        {
      ...
      } catch (execError)
      {
        ...
        */
        return handleError(res, execError as Error, "Error processing file.");
      }
    } catch (error)
    {
  ...
  router.post('/cleanup', authenticateUser, async (req, res) =>
  {
    try
    {
      ...
      const originalFilePath = path.join(TMP_CANVA_DIR, originalFilename);
      const processedFilePath = path.join(PROCESSED_DIR, processFile(originalFilename));
      let thumbnailFilePath: string | undefined;

      // If the file is a video, set the thumbnail path
      if (isVideo)
      {
        thumbnailFilePath = path.join(PROCESSED_DIR, `thumbnail_${path.parse(processedFilePath).name}.jpg`);
      }
  ...
  router.use('/tmp_canva', express.static(TMP_CANVA_DIR));
  router.use('/tmp', express.static(PROCESSED_DIR));
